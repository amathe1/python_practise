...
If you select 2, the program asks you to enter a new item. It adds what you have typed to the list.
...
If you select 4, the program asks you to enter the list number of an item to change, then asks you to enter the text. 
It changes specified item into specified text.
...
def change_item(db):
    item_number = input("Enter no of item to change : ")
    item = input("Enter new item : ")
    db[int(item_number)] = item

def delete_item(db):
    item_number = input("Enter item no to delete : ")
    db.pop(int(item_number))


    
def main():
    db = ["apple", "banana", "Orange"]
